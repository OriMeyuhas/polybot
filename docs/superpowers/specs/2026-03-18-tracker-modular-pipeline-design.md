...
├── book_recorder.py       # CLOB WebSocket — captures order book depth snapshots
...
CLOB WS → book_recorder → book_snapshots.csv
...
| book_depth_5c_bid | float | CLOB book query (total $ within 5c of best bid) |
| book_depth_5c_ask | float | CLOB book query (total $ within 5c of best ask) |
...
| spot_at_open | float | spot_recorder buffer |
| spot_at_close | float | spot_recorder buffer |
...
- On each new trade:
  ...
  - Queries CLOB REST for book depth at fill time
...
- After a market's `window_end_epoch` passes, polls Polymarket for settlement outcome
...
- Looks up spot buffer for `spot_at_open` and `spot_at_close`
...
- Connects to Polymarket CLOB WebSocket for order book updates
- Maintains in-memory book per active market (from `TrackerState.active_markets`)
- Every 5 seconds: snapshots each active market's book to `book_snapshots.csv`
  - Computes depth at 1c/5c/10c levels by summing order sizes within price bands
...
- `polybot/config.py` — `TrackerConfig` extended with new fields
...
1. **Settlement tracking + PnL computation** — foundation for all analysis
2. **Spot price trajectory** — SpotBuffer + continuous recording
3. **Enhanced trade_poller** — 2s polling, spot deltas, book depth at fill time
4. **Book recorder** — CLOB WebSocket depth snapshots
5. **Runner + CSV writer + migration** — wire everything together
6. **Dashboard** — minimal status view (last priority)
...
- Unit tests for `SpotBuffer` (record, get_price_at with various lookback windows)
